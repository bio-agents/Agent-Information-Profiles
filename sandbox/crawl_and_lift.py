...
def rdfize(json_entry):
    ...
    try:
        ...
        #entry['@type'] = ['bsc:Agent','sc:SoftwareApplication']
        entry['@type'] = ['sc:SoftwareApplication']
        entry['applicationCategory'] = 'Computational science agent'
...
def crawl_bioagents(collection="", limit=-1, dump=False):
    ...
    try:
        ...
        #url = 'https://bio.agents/api/agent/?page=1&?format=json'
        #r = requests.get(url)
        #print(r.json())
        #count_json = r.json()
        ...
        while has_next_page :
            if collection:
                req = http.request('GET', 'https://bio.agents/api/agent/?collectionID=' + str(collection) + '&page=' + str(i) + '&format=json')
            else:
            ...
            for agent in entry['list']:
                ...
                if dump:
                    ...
                    os.makedirs('./bio.agents.dataset/'+agent['bioagentsID'], exist_ok=True)
                    temp_graph.serialize(format="json-ld", auto_compact=True, destination=str('./bio.agents.dataset/'+agent['bioagentsID']+'/'+agent['bioagentsID']+'.bioschemas.jsonld'))

                    # with open('./bio.agents.dataset/'+agent['bioagentsID']+'/'+agent['bioagentsID']+'.json', 'w') as outfile:
                    #     json.dump(entry['list'], outfile, indent=True, sort_keys=True)
                    
                nb_agents += 1
                progress = nb_agents * 100 / count
                if (nb_agents % 500 == 0) :
                    print(str(round(progress))+" % done")
                if ((limit != -1) and (nb_agents >= limit)):
                    return graph
            i += 1
...
if __name__ == '__main__':
    crawl_bioagents(limit=100, dump=True)